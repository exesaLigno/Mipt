...
int main(int argc, char **argv)
{
    ...
    for (int line = 2; line < height; line++)
    {
        if (rank != comm_size - 1 and line > 3)
            MPI_Recv(&(grid[line - 2][stop_col + 1]), 3, MPI_DOUBLE, rank + 1, DATA_TRANSFER_TAG, MPI_COMM_WORLD, &status);

        for (int column = stop_col; column >= start_col; column--)
            grid[line][column] = sin(5 * grid[line - 2][column + 3]);

        if (rank != 0 and line < height - 2)
            MPI_Send(&(grid[line][start_col]), 3, MPI_DOUBLE, rank - 1, DATA_TRANSFER_TAG, MPI_COMM_WORLD);
    }

    if (rank != 0)
    {
        for (int line = 2; line < height; line++)
            MPI_Send(&(grid[line][start_col]), stop_col - start_col + 1, MPI_DOUBLE, 0, DATA_TRANSFER_TAG, MPI_COMM_WORLD);
    }
    ...
    else
    {
        for (int sender_rank = 1; sender_rank < comm_size; sender_rank++)
        {
            ...
            for (int line = 2; line < height; line++)
                MPI_Recv(&(grid[line][sender_start_col]), sender_stop_col - sender_start_col + 1, MPI_DOUBLE, sender_rank, DATA_TRANSFER_TAG, MPI_COMM_WORLD, &status);
        }
    }
